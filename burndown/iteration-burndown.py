...
if len(sys.argv)==1:
    print("\nError, usage: python iteration-burndown.py burndown.txt\n"+
            "burndown.txt should have the format\n"+
            "iteration points\n" +
            "iteration points...\n")

else:
    dat = np.loadtxt(sys.argv[1],skiprows=1)
    if len(np.shape(dat))==1: #1 dimension
        ...
        plt.ylabel(r'Story points',fontsize=24)
        ...
        for point in range(len(points))[1:]:
            if points[point]!=points[point-1]:
                if point==len(points)-1 or points[point] > points[point+1]:
                    ...
                    plt.text(iterations[point], points[point]-5,str(int(points[point])),fontsize=16)

        plt.legend(fontsize=24) 
        plt.savefig("iteration-burndown.png")
        plt.show()
